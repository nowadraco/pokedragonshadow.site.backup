document.addEventListener('DOMContentLoaded', async (event) => {
    try {
        ...
        if (sortedLinks.length < 2) {
            throw new Error('Não há posts suficientes para exibir a penúltima postagem.');
        }

        const secondLastPost = sortedLinks[1];
        console.log('Penúltimo post:', secondLastPost);

        const postResponse = await fetch(secondLastPost);
        if (!postResponse.ok) {
        ...
        const postLink = secondLastPost;

        console.log('Processando post:', { title, firstImage, firstParagraph, secondParagraph, postLink });
        ...
        document.querySelector('#penultimaPostagem').appendChild(mainPageElement);
        console.log("Penúltimo post adicionado à página principal.");
    } catch (error) {
        console.error('Erro ao buscar o penúltimo post:', error);
    }
});
